...
namespace WinFormsSampleControls.EulerDiagram {
  [ToolboxItem(false)]
  public partial class EulerDiagramControl : System.Windows.Forms.UserControl {
    private Diagram _Diagram;

    public EulerDiagramControl() {
      InitializeComponent();

      diagramControl1.AfterRender = Setup;
      goWebBrowser1.Html = @"
        <p>
          A sample of a Euler diagram: that is, a means of representing various sets and their relationships with one another. Euler 
    ...
    private void Setup() {
      ...
      _Diagram.NodeTemplate =
        new Node(PanelLayoutAuto.Instance) { LocationSpot = Spot.Center }
        .Bind(new Binding("Location", "Loc", Point.Parse))
        .Add(
      ...
      _Diagram.NodeTemplateMap.Add("center",
        new Node(PanelLayoutSpot.Instance) { LocationSpot = Spot.Center }
        .Bind(new Binding("Location", "Loc", Point.Parse))
        .Add(
          new Shape() {
            Figure = "Circle",
