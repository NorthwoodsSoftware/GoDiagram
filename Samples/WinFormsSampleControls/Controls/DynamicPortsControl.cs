...
namespace WinFormsSampleControls.DynamicPorts {
  [ToolboxItem(false)]
  public partial class DynamicPortsControl : System.Windows.Forms.UserControl {
...
";
  ...
  ""LinkDataSource"": [
    ...
    private void AddPort(string side) {
      ...
      foreach (var n in myDiagram.Selection) {
        ...
        if (arr != null) {
          ...
          myDiagram.Model.InsertListItem(arr, -1, newportdata);
        }
      }
    ...
    private void RemovePort(GraphObject port) {
      ...
      for (var i = 0; i < arr.Count; i++) {
        if (arr[i].PortId == pid) {
          myDiagram.Model.RemoveListItem(arr, i);
          break;
        }
      }
    ...
    private void RemoveAll(GraphObject port) {
      ...
      myDiagram.Model.Set(nodedata, side + "List", new List<PortData>());  // an empty list
    ...
    private void ChangeColor(GraphObject port) {
      ...
      myDiagram.Model.Set(data, "PortColor", GetPortColor());
